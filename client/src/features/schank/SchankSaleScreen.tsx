...
import { JetonType, MenuItem } from '@/types';
...
export function SchankSaleScreen() {
  ...
  useEffect(() => {
    ...
    if (isJeton) jetonTypesApi.getJetonTypes().then(setJetonTypes).catch(() => {});
  }, [isLoaded, isJeton]);
                  ...
                  {isJeton ? (
                    item.jeton_type_id != null ? (
                      <span className="inline-flex items-center gap-1.5 text-sm font-medium">
                        <span className="w-3 h-3 rounded-full inline-block border border-slate-200" style={{ background: jetonTypesById.get(item.jeton_type_id)?.color }} />
                        {jetonTypesById.get(item.jeton_type_id)?.name}
                      </span>
                    ) : (
                ...
                {isJeton ? (
                  item.jeton_type_id != null ? (
                    <div className="text-sm text-slate-500 flex items-center gap-1.5">
                      <span className="w-2.5 h-2.5 rounded-full inline-block border border-slate-200" style={{ background: jetonTypesById.get(item.jeton_type_id)?.color }} />
                      {jetonTypesById.get(item.jeton_type_id)?.name}
                    </div>
                  ) : (
